  ...
  return function configure (){
    ...
    if (app.config.sockets.adapterOptions.db) {
      ...
      try {
        ...
        if (!isInteger || app.config.sockets.adapterOptions.db > 1000000 || app.config.sockets.adapterOptions.db < 0) {
          throw new Error('Must be an integer between 0 and 1,000,000 -- but instead got: '+app.config.sockets.adapterOptions.db);
        }

      } catch (e) {
        throw ERRORPACK.CONFIG('The configured value for sails.config.sockets.adapterOptions.db (`'+app.config.sockets.adapterOptions.db+'`) is invalid: '+e.message);
      }
      // --•
