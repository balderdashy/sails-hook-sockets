  ...
  return function configure (){
    ...
    _.each(['host', 'port', 'pass', 'db', 'pubClient', 'subClient', 'subEvent', 'socket', 'key', 'url'], function(adapterOptionKey) {
      if (app.config.sockets[adapterOptionKey]) {
        app.log.debug('`sails.config.sockets.' + adapterOptionKey + '` is deprecated; use `sails.config.sockets.adapterOptions.' + adapterOptionKey + '` instead.');
        app.config.sockets.adapterOptions[adapterOptionKey] = app.config.sockets[adapterOptionKey];
        delete app.config.sockets[adapterOptionKey];
      }
    });
    ...
    if (app.config.sockets.adapterOptions.db) {
      ...
      try {
        ...
        var isInteger = (Math.floor(app.config.sockets.db) === app.config.sockets.db);
        if (!_.isNumber(app.config.sockets.db)) {
          throw new Error('Must be a number -- but instead got: '+app.config.sockets.db+' (a '+typeof app.config.sockets.db+')');
        }
        if (!isInteger || app.config.sockets.db > 1000000 || app.config.sockets.db < 0) {
          throw new Error('Must be an integer between 0 and 1,000,000 -- but instead got: '+app.config.sockets.db);
        }

      } catch (e) {
        throw ERRORPACK.CONFIG('The configured value for sails.config.sockets.db (`'+app.config.sockets.db+'`) is invalid: '+e.message);
      }
      // --•
