    ...
    if (app.config.sockets.db) {
      ...
      try {
        ...
        if (!_.isNumber(app.config.sockets.db)) {
          throw new Error('sails.config.sockets.db must be a number -- but instead got: '+app.config.sockets.db+' (a '+typeof app.config.sockets.db+')');
        }
        if (!isInteger || app.config.sockets.db > 1000000 || app.config.sockets.db < 0) {
          throw new Error('sails.config.sockets.db must be an integer between 0 and 1,000,000 -- but instead got: '+app.config.sockets.db);
        }
