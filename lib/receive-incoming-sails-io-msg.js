...
var ParseMessageError = require('./errors/ParseMessageError');
...
module.exports = function ToReceiveIncomingSailsIOMsg(app) {
  ...
  return function receiveIncomingSailsIOMsg(options) {
    ...
    var responseContext = {
      ...
      _clientCallback: function _clientCallback(clientRes) {
        ...
        try {
          ...
          // TODO: emit warning on our sails `app` object
          console.warn('Could not persist res.headers["set-cookie"] into the socket handshake');
        }
    ...
    function respondWithParseError (detailedErrorMsg) {
      ...
      var error = ParseMessageError('Failed to parse incoming socket.io request.');
