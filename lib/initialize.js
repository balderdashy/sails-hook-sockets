      ...
      prepareAdapter(function (err){
        ...
        io.use(function(socket, next){
          loadSessionFromSocket(socket.handshake, app, function (err) {
            ...
            if (err) {
              app.log.verbose('A socket is being allowed to connect, but the session could not be loaded.  Created an empty, one-time session to use for the life of the socket connection.  Details:\n',err);
            }
            return next();
          });
        });
