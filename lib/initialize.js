...
module.exports = function ToInitialize(app) {
  ...
  return function initialize (done) {
    ...
    (function waitForOtherHooks(next){
      ...
      prepareAdapter({
        adapter: app.config.sockets.adapter
      }, function (err){
        if (err) return done(err);

        // If we ever need pre-connection socket.io middleware, do that here:
        // (currently `beforeConnect` just uses engine.io's `allowRequest` directly)
