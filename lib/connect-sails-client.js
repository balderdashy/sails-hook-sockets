...
module.exports = function connectSailsSocketClient(app) {
  ...
  app.io.of('/sails').on('connection', function(socket) {
    ...
    app.log.verbose('sails-hook-sockets :: connected to Sails admin message bus.');
  ...
  var socket = socketIOClient("http://localhost:" + app.config.port + "/sails?_isSailsAdminSocket_=true", {multiplex: false, transports: app.config.sockets.transports});
