  ...
  return function prepareAdapter(cb){
    ...
    (function _getAdapter(proceed){
      ...
      try {
        ...
        // If they're using socket.io-redis, attempt to figure out which version they have and verify that it's compatible.
        if (adapterModuleName === 'socket.io-redis') {
          try {
            ...
            if (semver.lt(SocketIOAdapterVersion, MINIMUM_SIO_REDIS_VERSION)) {
              throw flaverr({name: 'userError', code: 'E_SOCKET_IO_REDIS_VERSION_TOO_LOW'}, new Error('Please install socket.io-redis version ' + MINIMUM_SIO_REDIS_VERSION + '! (npm install --save socket.io-redis@5.2.0)'));
            }
          }
          catch (e) {
            if (e.code === 'E_SOCKET_IO_REDIS_VERSION_TOO_LOW') { throw e; }
            // If there's some other error trying to read the package.json of socket.io-redis, that is weird, but let's hope for the best
