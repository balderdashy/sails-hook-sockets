...
module.exports = function (app){
  ...
  return function prepareDriver(adapterDef, cb){
    ...
    var url = adapterConfig.url || Redis.createConnectionUrl(_.pick(adapterConfig, ['host', 'port', 'password', 'database'])).execSync();

    // Create a Redis connection manager.
    Redis.createManager({
      connectionString: url,
      meta: _.extend({return_buffers: true}, _.omit(adapterConfig, ['host', 'port', 'password', 'database', 'url'])),
