...
module.exports = function (app){
  ...
  return function prepareDriver(adapterDef, cb){
    ...
    // If the `pass` or `db` adapter config options are set, we need to create our
    // own Redis clients to use for pub/sub, because when socket.io-redis creates
    // clients for us it doesn't use our options.  If we don't need these options,
    // we can bail out now and just let socket.io-redis create the clients.
