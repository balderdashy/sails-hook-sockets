...
module.exports = function ToHandleNewConnection(app){
  ...
  return function onConnect (socket){
    ...
    (function runOnConnectListener(){
      ...
      app.log.error('`sails.config.sockets.onConnect` has been deprecated, and support will be removed in an upcoming release. See the v0.11 migration guide for more information.');
      app.log.error('(running it for you this time)');
    ...
    socket.on('disconnect', function onSocketDisconnect(){
      ...
      _loadSessionFromSocket(socket, app, function finishedLoading(err, session){
        ...
        app.config.sockets.afterDisconnect(session, socket, function (err) {
          ...
          if (!_.isFunction(session.save)) {
            app.log.warn('Could not save session in `sails.config.sockets.afterDisconnect`- session was never loaded.');
            return;
          }
