...
function _loadSessionFromSocket(socket, app, cb) {
  ...
  if (!socket.handshake.headers.cookie){
    socket.handshake.headers.cookie = app.config.session.key + '=' + uuid.v1();
  }
  ...
  app.log.verbose('Generated a one-time-use cookie:', generatedCookie,'and saved it on the socket handshake.');
  ...
  // // Generate session, then return the sid
  // app.session.set(function (err, newSession){
  //   if (err) {
  ...
  //   // Set cookie in socket handshake
  //   newSession.id;
  // });
